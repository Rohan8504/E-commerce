...
// Image Slider
const sliderImgSelector = document.querySelectorAll(
...
const sliderFocusImgSelector = document.querySelectorAll(
  ".slider__selector--focus-img img"
);

sliderFocusImgSelector.forEach((el) => {
  el.addEventListener("click", () => {
    sliderImgFocus.src = `/images/image-product-${el.getAttribute(
      "focus-value"
    )}.jpg`;
    counter = el.id;
...
const updateCart = () => {
  ...
  let htmlCode = `<img class="cart__product--img" src="./images/image-product-1-thumbnail.jpg" alt="">
